...
module core (
    ...
    // muldiv stall: a start pulse fires the cycle we first see is_muldiv (and are not already
    // stalled on a prior one); pc holds and regfile wen is gated until done fires.
    logic muldiv_busy, muldiv_done, muldiv_start, stall;
    assign stall       = muldiv_busy;
    assign muldiv_start = is_muldiv && !muldiv_busy && !muldiv_done;

    muldiv u_muldiv (
        .clk(clk), .rst(rst), .a(rdata1), .b(rdata2), .op(muldiv_op), .is_word(is_word),
        .start(muldiv_start), .busy(muldiv_busy), .done(muldiv_done), .y(muldiv_y)
    );
    ...
    logic regfile_wen;
    assign regfile_wen = reg_write && (!is_muldiv || muldiv_done);

    regfile u_regfile (
        .clk(clk), .rs1(rs1), .rs2(rs2), .rd(rd),
        .wdata(wdata), .wen(regfile_wen), .rdata1(rdata1), .rdata2(rdata2)
    );

    always_comb begin
        if (is_muldiv && !muldiv_done) pc_next = pc; // hold pc while the divider/multiplier is busy
        else if (is_jal)               pc_next = pc + imm;
